...
- `useStreamingStore` — non-persisted; play flow, episode context, preloading, request cancellation
...
| `lib/stores/` | Zustand stores: settings, streaming, vlc, preview, selection |
...
| `app/(public)/` | Public routes: landing, login, signup, forgot-password, reset-password, status |
| `app/api/` | API routes: addon proxy, auth, health, progress, subtitles (VLC proxy), trakt OAuth |
