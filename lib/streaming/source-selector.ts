...
/**
 * Select the best source for binge-watching (next episode / after user stream change).
 *
 * Strategy:
...
export function selectBestSourceForBinge(
    ...
    if (bingeGroupSources.length > 0) {
        // Run normal selection within the binge-group subset.  Pass the options but
        // clear `preferredBingeGroup` to avoid double-applying its scoring bonus.
        ...
        if (result.hasMatches) {
            // Build allSorted from full source list (so in-player picker still shows
            // everything), but put the binge result's source first.
