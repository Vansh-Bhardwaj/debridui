...
import { SplashErrorScreen } from "@/components/auth/splash-error-screen";
...
export function AuthProvider({ children }: AuthProviderProps) {
    ...
    const currentAccountId = useMemo(() => {
        ...
        if (selectedAccountId && userAccounts.some((acc: UserAccount) => acc.id === selectedAccountId)) {
            return selectedAccountId;
        }

        const savedId = typeof window !== "undefined" ? localStorage.getItem("selected-account-id") : null;
        if (savedId && userAccounts.some((acc: UserAccount) => acc.id === savedId)) {
            return savedId;
        }
    ...
    const currentAccount = useMemo(
        () => userAccounts.find((acc: UserAccount) => acc.id === currentAccountId) || null,
    ...
    // Logout function - always clears local state regardless of server response
    const logout = useCallback(async () => {
        ...
        try {
            ...
            // Use hard navigation to ensure all auth state is reset
