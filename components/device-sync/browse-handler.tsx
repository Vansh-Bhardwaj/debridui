...
export function BrowseHandler() {
    ...
    useEffect(() => {
        ...
        const handler = async (e: Event) => {
            ...
            const toFileItem = (f: { id: string; name: string; size: number; status: string; progress?: number; createdAt?: string | Date }): BrowseFileItem => ({
                id: f.id,
                name: f.name,
                size: f.size,
                status: f.status,
            ...
            try {
                if (request.action === "list-files") {
                    ...
                    response = {
                        ...
                        files: result.files.map(toFileItem),
                        total: result.total,
